...
объекты по определенному правилу. Объекты разделены пробелами. Преобразование
делать по возможности через словарь. Для упрощения под выводом числа прописью
подразумевается последовательный вывод всех цифр числа. Регулярные выражения
использовать нельзя.
Вариант 17.
...
# Cловарь, отображающий цифры (0-9) на их словесные представления
digits_map = {
...
def process_sequence(filename):
    ...
    # Инициализация текущей невозрастающей подпоследовательности первым числом
